...
// TO DO -- write this function
  ...
  while(!toDo.empty()){
    ...
    toDo.pop();
    for (pair<int, double> &nb : database[popIndex].neighbors){
      if(database[nb.first].isVisited){
        continue;
      }
      database[nb.first].cost = database[popIndex].cost + 1;
      database[nb.first].prev = popIndex;
      toDo.push(nb.first);
      database[nb.first].isVisited = true;
      if(nb.first == iEnd){
        while(!toDo.empty()){
          toDo.pop();
        }
